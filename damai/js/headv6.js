...
function getNickName() {
    ...
    if (value) {
        ...
        strHtml += '<a class="dm-exit" title="退出" href="//www.damai.cn/redirect.aspx?type=loginout">退出</a>';
    }
    else {
    ...
    if (value) {
        jQuery.ajax({
            type: "get",
            url: "//piao.damai.cn/ajax/getUserGrade.html",
            dataType: "jsonp",
            jsonp: "jsonCallback",
            success: function (data) {
                if (data && data.Status && data.Status == 200 && data.Data.g > 0) {
                    jQuery("#headerUserinfo span.dm-name").after("<a class='vip m" + data.Data.g + "' target='_blank' href='//vip.damai.cn/'></a>");
                }
            }
        });
    }
...
function initSearch() {
    ...
    function suggest(v) {
        ...
        var url = "//search.damai.cn/suggest.html" + "?callback=suggestJsonp&keyword=" + encodeURIComponent(txt);
    ...
    dui.on(dui.$("#search"), "click", function () {
        ...
        if (searchText == '请输入演出，艺人，场馆名称…') searchText = "";
        doSearch(searchText);
    }, true);
    ...
    dui.on(dui.$("#searchText"), "focus", function (event) {
        ...
        if (value == "请输入演出，艺人，场馆名称…" || value == key) { value = ""; text.val(value); }
